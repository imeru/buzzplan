...
| roomvent-2026 | RoomVent 2026 | parser_roomvent.py | 59세션/269발표 | **default**. tz Europe/Prague. 단일 건물 `CTU`, 방으로만 구분. 발표번호 없어 paper_no는 `<세션ID>-<순번>`. **venue.maps 보유**(2층만, 축척 0.071 m/px) |
...
- **2층만 있다.** 1층의 B 168과 B 169는 핀이 없어 계층 규칙(같은 건물 4분)으로 넘어간다.
  부분 지도는 안전하게 동작한다. 핀이 있는 방만 지도로 계산되고 나머지는 이전과 같다
