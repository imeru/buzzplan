...
이 표의 숫자가 바뀌는 작업을 했으면 이 파일도 같이 갱신한다.
...
- 부팅 오류 페이지, 학회명 수정 툴팁 등 극저빈도 문구는 한국어 고정 (i18n 미적용)

## 13. 백로그 (우선순위 순. 착수 전 사용자에게 한 줄 확인)

1. **발표 시작 5분 전 알림**: 1차는 ICS에 VALARM 포함(반나절, 다음 학회에서 바로 체감),
   Notification API는 그 뒤 검토
2. **파서 레이아웃 헬퍼 통합**(group_rows, consolidate_split_chars, is_skip_row →
   parser_utils.py): 2026 동계 SAREK 추가 전에 해두면 효율
3. **sarek-2025-summer 포스터 추가** (Tier 2 방식): 지난 학회라 아카이브 완결성 목적.
