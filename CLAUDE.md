...
| roomvent-2026 | RoomVent 2026 | parser_roomvent.py | 59세션/269발표 | **default**. tz Europe/Prague. 단일 건물 `CTU`, 방으로만 구분. 발표번호 없어 paper_no는 `<세션ID>-<순번>`. **venue.maps 보유**(1층과 2층, 축척 0.071 m/px) |
...
- **삭제 전파**: 선택 해제해도 stamps가 남아 tombstone(s:0) 역할.
  그래서 **서버 `keys` 수와 받는 기기의 `stamps` 수는 정상적으로 어긋난다.**
  `applyRemoteItems`가 내용 빈 tombstone을 건너뛰므로(받을 것이 없다) 그 키는 수신 기기의
  stamps에 생기지 않는다. 해제를 직접 한 기기에만 기록이 남는다
  (2026-09-11 관측: 서버 46키 중 내용있음 44. 크롬 `stamps=46`, 사파리 `stamps=44`. 설계대로다)
...
- **그 서버 읽기에 타임아웃과 재시도를 건다** (`getServerWithTimeout`. 첫 시도
  ...
  - **사파리 실측(2026-09-11, 12초 타임아웃 적용 후)**: 첫 읽기가 `pending#1 → ok#1 1505ms`로
    첫 시도에 성공한다. 패널 자신의 읽기 1021ms, 계정 스캔 239ms. 5초 타임아웃 시절의
    `timeout ms=5002`는 타임아웃 값이 짧았던 탓이고, 전송 계층을 바꿀 문제가 아니었다.
    `experimentalForceLongPolling`은 필요 없다. 단 이 측정은 persistence 캐시가 이미 있는
    상태의 값이다. 캐시가 빈 최초 로그인은 더 걸릴 수 있으나 12초 + 재시도 4회(예산 53초)와
    `listener-ok` 복구로 덮인다
...
- **동기화 진단 패널 (`?debug=sync`)**: `syncDebugOn`/`mountSyncDiag`/`runSyncDiag`.
  ...
  수백 ms에 끝나면 본 읽기는 느린 것이 아니라 멈춘 것이라고 가를 수 있다.
  **상태를 담은 줄은 전부 `liveLine(fn)`으로 넣어 네트워크 대기가 끝난 뒤 일괄로 다시 쓴다**
  (`time`, `local`, `flags`, `server read`, `read log`). 대기 전 값만 찍으면 한 화면의 줄들이
  서로 다른 시점을 가리키면서 그 사실이 표시되지 않는다. 이 함정에 2026-09-11 하루에 두 번
  걸렸다. 처음에는 `server read`와 `this conf`의 시점이 어긋나 스냅샷 하나를 판정 불가로
  버렸고, 고치면서 `flags`를 갱신 목록에서 빼 두어 **읽기는 `ok`인데 `serverSeen=false`인
  모순 스냅샷**이 나왔다(읽기가 대기 중에 성공했고 flags만 옛 값이었다). 새 상태 줄을 추가할
  때 `L.push`를 쓰면 같은 일이 반복된다. `time` 줄에는 대기가 길었으면 경과가 붙고,
  `read log` 끝에는 `| now +N.Ns`가 붙어 전이가 검사 전에 끝난 것인지 드러난다.
...
**roomvent-2026 지도의 유래와 한계** (2026-09-16): 학회 현장에서 촬영한 주최측
안내도("UPPER LEVEL") 한 장이다. 프로그램북과 학회 웹사이트에는 도면이 없고, CTU는
평면도를 보안문서로 취급해 온라인에 공개하지 않는다. 현장 촬영이 유일한 경로였다.
- **두 장이고, 1층은 2층 좌표계로 다시 만든 것이다.** 1층(ENTRANCE LEVEL)과
  2층(UPPER LEVEL) 안내도를 따로 촬영했는데 범위와 배율이 달랐다. 1층 도면에는 A동이
  없고 전체가 약 1.2배 크게 그려져 있었다. `planarMeters`가 같은 건물의 층 도면을 같은
  좌표계로 보고 좌표를 그대로 빼므로, 그대로 넣으면 층 사이 거리가 틀어진다. 그래서
  1층 사진을 스케일 0.4436으로 축소해 2층과 같은 2000x960 캔버스에 올렸다. 기준점은
  B동 좌상단 외벽이고, 스케일은 B동 폭과 높이에서 각각 구한 값(0.4416, 0.4455)의
  평균이다. 두 값의 불일치가 0.9%라 촬영 왜곡이 작다고 보았다
- **정렬은 두 층에 공통인 구조물로 검증했다.** B동 좌측 외벽 1px, C동 상단 외벽 8px
  (0.6m) 차이다. 두 도면을 빨강과 파랑으로 겹쳐 그려 육안으로도 확인했다. 같은 일을
  다시 할 때는 이 순서를 따를 것이다. 외벽 자동 검출은 탐색 범위 경계에 걸려 엉뚱한
  선을 잡기 쉬우므로, 검출값은 반드시 겹침 이미지로 확인한다
- 도면에 없는 방은 핀을 넣지 않으면 된다. **부분 지도는 안전하게 동작한다.** 핀이 있는
  방만 지도로 계산되고 나머지는 계층 규칙으로 넘어간다
- **축척은 OpenStreetMap 실내 데이터(ODbL)로 잡았다.** 촬영본에는 축척 표시가 없었다.
  OSM에 이 건물의 방 폴리곤이 KOS 방 번호(`TH:C-215` 등)와 함께 매핑되어 있어, 가장 긴
  기선인 C 215와 A 230 사이 75.5 m를 기준으로 `width_meters` 142를 산출했다. 나머지
  세 쌍으로 교차 검증한 오차는 1.2%에서 4.7% 사이다. 이 방법은 촬영본에 축척이 없을 때
  쓸 수 있다. Overpass API는 User-Agent가 없으면 406을 돌려주므로 반드시 붙일 것
- **B동과 C동 사이는 과소평가된다.** 직선거리 모델이라 실제 연결통로 경로를 모른다.
  B 286에서 C 215가 1분으로 나오는데 실제로는 더 걸린다. `detour`를 1.3에서 1.5로 올려
  두었으나 근본 해결은 아니다. 실측값을 받으면 그것을 쓰는 편이 낫다. 다만 모든 세션의
  `building`이 `CTU`로 같아 `venue.walk.pairs`는 쓸 수 없다(pairs는 건물이 다를 때만
  적용된다). 방별 실측을 반영하려면 building을 `CTU A`/`CTU B`/`CTU C`로 나눠야 하는데,
  그러면 `_useRoomLevelBuilding`이 꺼져 회장 필터의 단위가 방에서 동으로 바뀐다
